...
def build_waveform_cache_spec(num_samples, generator_backend=None, psd_csv=None):
    ...
    generator_settings = {
        ...
        'generator_name': 'pycbc_modified_data_generator' if MODIFIED_GRAVITY else 'pycbc_data_generator',
        ...
        'approximant': 'IMRPhenomD' if MODIFIED_GRAVITY else 'IMRPhenomXP',
        'f_lower': 30.0 if MODIFIED_GRAVITY else 40.0,
        'time_resolution': 1.0 / 4096.0,
        'signal_length': 2.0,
        'f_final': 2048.0 if MODIFIED_GRAVITY else None,
        'lambda_g_range': [LAMBDA_G_MIN, LAMBDA_G_MAX] if MODIFIED_GRAVITY else None,
    ...
    mode_token = 'mg' if MODIFIED_GRAVITY else 'gr'
...
def prepare_pycbc_data(num_samples=10000, generator_backend=None, psd_csv=None):
    ...
    # If modified gravity is on, add lambda_g to the config so it is sampled
    # and passed through to the FD waveform generator.
    if MODIFIED_GRAVITY:
    ...
    try:
        ...
        # If MG is enabled, use pycbc_modified_data_generator.py
        if gen_backend == 'real_psd':
            ...
            if MODIFIED_GRAVITY:
                ...
                print("  MG utilities (_additional_phase, _generate_single_modified_waveform) active in data generation.")
            else:
...
NUM_TRAINING_SAMPLES = 30000  # Adjust this to control dataset size
...
MODEL_SAVE_PATH = f"dingo_N{samples_str}_F{NUM_FLOW_LAYERS}_C{CONTEXT_DIM}_H{HIDDEN_DIM}_E{NUM_EPOCHS}_{EMBEDDING_TYPE}_{noise_str}{whiten_str}_{PARAM_SET}{mg_str}.pt"
...
try:
    parameter_lines = [
        ...
        f"Waveform model type: {'Modified gravity waveform generation' if MODIFIED_GRAVITY else 'GR waveform generation'}",
        f"GENERATOR_BACKEND: {GENERATOR_BACKEND}",
        f"REAL_PSD_CSV: {REAL_PSD_CSV}",
        f"ADD_NOISE: {ADD_NOISE}",
        f"WHITEN: {WHITEN}",
        f"Noise generation mode: {'Detector noise added in generator' if ADD_NOISE else 'No detector noise (clean waveforms)'}",
...
# Split eval params into GR and modified-GR groups
