# train_model.py  (GPU variant)
# HPC/GPU training for the DINGO model. This is a standalone copy of the
# CPU trainer at ../cpu/train_model.py: the model, dataset loader, and
# helpers are kept identical up to the "GPU OVERRIDES" marker at the end,
# which reassigns DEVICE, extends DEFAULT_CONFIG, and swaps in an AMP +
# chunked-val-forward training loop that actually uses CUDA.
#
# Input:  <Pipeline>/Data/dataset.pt  (written by Data Generation/generate_dataset.py)
# Output: <Pipeline>/<model_name>_gpu.pt
...
# Placeholder — overridden to torch.cuda at the bottom of the file, once
# the GPU-specific block runs. Left as cpu here only so imports don't fail
# on machines with no CUDA build of torch when only the dataset loader is
# consumed (e.g. evaluate_model.py on a laptop).
...
# GPU OVERRIDES
#
# Everything above this line is a verbatim copy of the CPU trainer — the two
# files share the same model, dataset loader, embeddings, checkpoint shape,
# and helpers, so they never drift. The block below redefines DEVICE,
# extends DEFAULT_CONFIG with the GPU-only knobs, adds an AMP-aware
# training loop, and *reassigns* `run_training` at module scope so any
# importer (hp_search.py alongside this file, etc.) transparently gets the
# CUDA path. The CPU `train_dingo_model` above is left in the module as
# unreachable dead code rather than deleted, so the diff between the two
# train_model.py files stays a pure append.
# ==============================================================================

DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"[train_model.py (gpu)] PyTorch {torch.__version__}  device={DEVICE}")
if DEVICE.type == 'cuda':
    print(f"[train_model.py (gpu)] CUDA device: {torch.cuda.get_device_name(0)}  "
          f"({torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB)")


DEFAULT_CONFIG = {
    **DEFAULT_CONFIG,
    # Safe default for a 48 GB L40 with AMP bf16 + the full dataset resident
    # on device. Drop to 128 if conv1d with a long window OOMs.
    'batch_size':      256,
    'val_chunk_size':  256,
    'use_amp':         True,
    'allow_tf32':      True,
...
def _chunked_val_logprob(model, val_params, val_data, chunk_size,
    ...
    train because the model runs under a different dtype.
    """
    totals, counts = 0.0, 0
    model.eval()
...
def run_training(config=None):  # noqa: F811 — intentional shadow of CPU run_training
    """GPU-side counterpart to the CPU `run_training`. Same contract."""
    ...
    log_fh = None
    original_stdout = _sys_rt.stdout
    if cfg.get('log_file'):
        os.makedirs(os.path.dirname(cfg['log_file']) or '.', exist_ok=True)
        log_fh = open(cfg['log_file'], 'a', buffering=1)  # line-buffered
        _sys_rt.stdout = _Tee(original_stdout, log_fh)
        print(f"[run_training] Mirroring stdout → {cfg['log_file']}")

    try:
        return _run_training_impl_gpu(cfg, dev)
    finally:
        if log_fh is not None:
            _sys_rt.stdout = original_stdout
            log_fh.close()


def _run_training_impl_gpu(cfg, dev):
    import time
    print(f"\nLoading dataset from: {cfg['dataset_path']}")
    active_crop = resolve_crop_for_embedding(cfg)
    if active_crop is not None:
    ...
    param_names     = ds['param_names']
    metadata        = ds['metadata']
    data_shape      = tuple(ds['data_shape'][1:])
    ...
    print(f"  Train: {len(train_params)}  Val: {len(val_params)}  "
          f"Shape: (N, {num_detectors}, "
          f"{', '.join(str(s) for s in data_shape)})")
...
if __name__ == '__main__':
    ...
    # types / sizes / depths, use hp_search.py (sibling to this file).
    run_training({
        'dataset_path':           str(PIPELINE_ROOT / 'Data' / 'dataset.pt'),
        'merger_crop_half_width': 500,
        'embedding_type':         'lstm',
